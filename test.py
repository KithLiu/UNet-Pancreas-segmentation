...
class U_Net():
    def __init__(self):
        ...
        self.unet.compile(loss='binary_crossentropy',
                          optimizer=optimizer,
                          metrics=[self.metric_fun])   #loss=’mse’
        self.unet.summary()

    def build_unet(self, n_filters=16, dropout=0.2, batchnorm=True, padding='same'):
    ...
    def load_data(self):
        ...
        # 图片有三千张左右，按9:1进行分配
        return x_train[:6014, :, :], x_label[:6014, :, :], x_train[6014:, :, :], x_label[6014:, :, :]    # 5346
